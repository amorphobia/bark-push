    ...
    // Custom plugin to inject i18n metadata after build
    {
      ...
      closeBundle() {
        ...
        if (fs.existsSync(filePath)) {
          ...
          // Find the end of the userscript header (before the closing ==/UserScript==)
          const headerEnd = content.indexOf('// ==/UserScript==');
          
          if (headerEnd !== -1) {
            const i18nMetadata = `// @name:zh-CN    Bark 推送
// @name:zh-TW    Bark 推送
// @name:ja       Bark プッシュ
// @name:ko       Bark 푸시
// @description:zh-CN  向 Apple 设备发送通知信息
// @description:zh-TW  向 Apple 裝置發送通知訊息
// @description:ja     Apple デバイスに通知を送信します
// @description:ko     Apple 기기로 알림을 보냅니다
`;
            
            // Insert i18n metadata before the closing tag
            content = content.slice(0, headerEnd) + i18nMetadata + content.slice(headerEnd);
